...
class AdamOptimizer:
    ...
    # calculates one step of the logistic regression update
    def step(self, X, y, k, alpha, beta_1, beta_2):
       if self.v_t is None:
       ...
       X_mini = X[indices, :]
       y_mini = y[[indices]]
       self.t += 1 # update the self.time
       g_t = self.grad(X_mini, y_mini, self.score(X_mini)) # find minibatch gradient
       m_t = beta_1 * self.m_t + (1 - beta_1) * g_t # find first moment
       v_t = beta_2 * self.v_t + (1 - beta_2) * g_t**2 # find second moment
       m_hat = 1 / (1 - beta_1**self.t) * m_t # error corrected first moment
       v_hat = 1 / (1 - beta_2**self.t) * v_t # error corrected second moment
       self.w = self.w - (alpha * m_hat / (torch.sqrt(v_hat) + self.t)) # update weights